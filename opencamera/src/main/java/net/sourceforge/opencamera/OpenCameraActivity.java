 ...
 */
public class OpenCameraActivity extends Activity implements AudioListener.AudioListenerCallback {
	...
	protected void onCreate(Bundle savedInstanceState) {
	    ...
	    orientationEventListener = new OrientationEventListener(this) {
			...
			public void onOrientationChanged(int orientation) {
				OpenCameraActivity.this.mainUI.onOrientationChanged(orientation);
			}
        };
		if( MyDebug.LOG )
        ...
        new Thread(new Runnable() {
            public void run() {
                textToSpeech = new TextToSpeech(OpenCameraActivity.this, new TextToSpeech.OnInitListener() {
        			@Override
	...
	private void setProgressSeekbarScaled(SeekBar seekBar, double min_value, double max_value, double value) {
		...
		double frac = OpenCameraActivity.seekbarScalingInverse(scaling);
    ...
    public void updateGalleryIcon() {
		...
		new AsyncTask<Void, Void, Bitmap>() {
		    ...
		    protected Bitmap doInBackground(Void... params) {
				...
				KeyguardManager keyguard_manager = (KeyguardManager)OpenCameraActivity.this.getSystemService(Context.KEYGUARD_SERVICE);
		...
		alertDialog.setItems(items, new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				if( which == clear_index ) {
				    ...
				    new AlertDialog.Builder(OpenCameraActivity.this)
					...
					if( which >= 0 && which < history.size() ) {
						...
						preview.showToast(null, getResources().getString(R.string.changed_save_location) + "\n" + save_folder_name);
						SharedPreferences sharedPreferences = PreferenceManager.getDefaultSharedPreferences(OpenCameraActivity.this);
						SharedPreferences.Editor editor = sharedPreferences.edit();
						if( applicationInterface.getStorageUtils().isUsingSAF() )
    ...
    class MyGestureDetector extends SimpleOnGestureListener {
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            try {
        		...
        		final ViewConfiguration vc = ViewConfiguration.get(OpenCameraActivity.this);
		...
		}
		{
		    ...
		    focusSeekBar.setOnSeekBarChangeListener(new OnSeekBarChangeListener() {
				...
				public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
					...
					double scaling = OpenCameraActivity.seekbarScaling(frac);
		...
		if( ok ) {
			...
			.setOnDismissListener(new OnDismissListener() {
				public void onDismiss(DialogInterface dialog) {
					...
					ActivityCompat.requestPermissions(OpenCameraActivity.this, permissions_f, permission_code);
				}
			}).show();
		}
	}
